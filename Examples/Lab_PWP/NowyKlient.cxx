...
int main(int argc, char* argv[])
{
  ...
  while(true){
		...
		headerMsg->InitPack();
		int r = socket->Receive(headerMsg->GetPackPointer(), headerMsg->GetPackSize()); // odbierz paczke
		
		if(r == 0){ // empty
			socket->CloseSocket();
		}
		else{
			headerMsg->Unpack(); // rozpakuj
			ReceivePoint(socket, headerMsg); // wez punkty i wyswietl
		}
  }
