...
// odeslanie puntkow do klienta
int SendMessage(igtl::Socket * socket, igtl::PointElement::Pointer point){

	igtl::PointMessage::Pointer pointMsg;
	pointMsg = igtl::PointMessage::New();
	pointMsg->SetDeviceName("PointSender");    igtlFloat32 pos[3];
    point->GetPosition(pos);
	point->SetPosition(-pos[0], -pos[1], -pos[2]); // zmiana znaku
	pointMsg->AddPointElement(point);
	pointMsg->Pack(); // zapakuj

	igtl::Sleep(1000); // 1 sek
...
int ReceivePoint(igtl::Socket * socket, igtl::MessageHeader * header)
{
  ...
  if (c & igtl::MessageHeader::UNPACK_BODY) // if CRC check is OK
    ...
    for (int i = 0; i < nElements; i ++)
	  ...
	  // tu
	  SendMessage(socket, pointElement); // wyslij do klienta

      }

	  socket->CloseSocket(); // zamknij socket
